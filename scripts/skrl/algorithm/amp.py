...
class AMP(Agent):
    ...
    def _update(self, timestep: int, timesteps: int) -> None:
        ...
        motion_states = self.collect_reference_motions(self._amp_batch_size)
        print(f"Motion_states: {motion_states.shape}")
        self.motion_dataset.add_samples(states=motion_states)

        # compute combined rewards
        rewards = self.memory.get_tensor_by_name("rewards")
        amp_states = self.memory.get_tensor_by_name("amp_states")
        print(f"AMP_states: {amp_states.shape}")
