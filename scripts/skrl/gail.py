...
class GAIL(AMP):
    def __init__(
        ...
        collect_observation: Callable[[], torch.Tensor],
    ) -> None:
    ...
    def _update(self, timestep: int, timesteps: int) -> None:
        ...
        with torch.no_grad(), torch.autocast(device_type=self._device_type, enabled=self._mixed_precision):
            amp_logits, _, _ = self.discriminator.act(
                {"states": self._amp_state_preprocessor(amp_states), "taken_actions": actions}, role="discriminator"
            )
        ...
        if len(self.reply_buffer):
            ...
            sampled_replay_batches = [[batches[self.tensors_names.index(elem)] for elem in ["amp_states", "actions"]] for batches in sampled_batches]
            ...
            for batch_index, (
                ...
                with torch.autocast(device_type=self._device_type, enabled=self._mixed_precision):
                    ...
                    if self._discriminator_batch_size:
                        ...
                        sampled_actions = sampled_actions[0 : self._discriminator_batch_size]
                        ...
                        sampled_replay_actions = sampled_replay_batches[batch_index][1][0 : self._discriminator_batch_size]
                        ...
                        sampled_motion_actions = sampled_motion_batches[batch_index][1][0 : self._discriminator_batch_size]
                    else:
                        ...
                        sampled_replay_actions = sampled_replay_batches[batch_index][1]

                        sampled_amp_motion_states = self._amp_state_preprocessor(
                            sampled_motion_batches[batch_index][0], train=True
                        )
                        sampled_motion_actions = sampled_motion_batches[batch_index][1]

                    sampled_amp_motion_states.requires_grad_(True)
                    amp_logits, _, _ = self.discriminator.act(
                        {"states": sampled_amp_states, "taken_actions": sampled_actions}, role="discriminator"
                    )
        ...
        self.reply_buffer.add_samples(
            ...
            actions=sampled_actions.view(-1, sampled_actions.shape[-1]),
        )
